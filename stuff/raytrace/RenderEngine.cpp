  ...
  if(argc > 1)
  {
    for(int i = 1; i < argc; ++i)
    {
      ...
      if(char_traits<char>::compare(filename.c_str(), "cornell", 7) == 0)
        ...
        transform = Matrix4x4::translate(make_float3(10.0f, 2.0f, -12.0f))*Matrix4x4::rotate(0.5f, make_float3(0.0f, 0.25f, 0.0f));
...
void RenderEngine::render() {
    ...
    for(int y = 0; y < static_cast<int>(res.y); ++y) {
        ...
        for (int x = 0; x < static_cast<int>(res.x); ++x) {
            /*
            * We create a pixel from the "tracer's" "compute_pixel" function.
            * Then we add that pixel into our image which (for some reason) is
            * just a one-dimensionel vector instead of a matrix, that is why we
            * add the entire row_dimension in the indexing.
            */
            float3 pixel = tracer.compute_pixel(x, y);
            image[y * res.x + x] = pixel;
