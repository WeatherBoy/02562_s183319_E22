...
bool BspTree::closest_hit(Ray& r, HitInfo& hit) const {
    ...
    Accelerator::closest_plane(r, hit);

    /*
    * We call intersect_min_max because it either returns false or it 
    * decreases our valid search space. If it returns false, then we
    * just return false, otherwise we try to call our intersect_node 
    * function.
    * The intersect_node function takes a BspNode, which is initialized
    * with the BspTree (see the init).
    */
...
bool BspTree::any_hit(Ray& r, HitInfo& hit) const {
    ...
    /*
    * Same reasoning as with the closest hit function.
    */
    if (!Accelerator::any_plane(r, hit)) {
        ...
        * in either case we return the hit.has_hit, because it is 
        * updated in both any_plane() and intersect_node().
        */
        if (intersect_min_max(r)) {
            intersect_node(r, hit, *root);
