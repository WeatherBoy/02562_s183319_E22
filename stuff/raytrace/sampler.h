...
inline optix::float3 sample_cosine_weighted(const optix::float3& normal) {
    ...
    /*
    * Implementing the sample_cosine_weighted function as based on the slide
    * 'Sampling a Cosine-Weighted hemisphere' (week 7, slide 10).
    * 
    * Here I declare the two xi-values, by use of the function
    * mt_random_half_open() as described in the slide. Then I calculate phi,
    * as well as cos to theta. Sine of theta is then found simply through
    * trigonometric relations.
    * Finally the cosine weighted normal is found from the calculated values
    * and the function spherical_direction(). It is changed from tangent spaced
    * to world space, by using the function rotate_to_normal().
    */
    double const xi_1 = mt_random_half_open();
    double const xi_2 = mt_random_half_open();

    double const phi = 2 * M_PIf * xi_2;
    double const cos_theta = sqrt(1 - xi_1);
    double const sin_theta = sqrt(xi_1);

    optix::float3 cosine_weighted_normal = spherical_direction(sin_theta, cos_theta, phi);
    rotate_to_normal(normal, cosine_weighted_normal);

    return cosine_weighted_normal;
}
