...
bool Accelerator::closest_hit(optix::Ray& r, HitInfo& hit) const {
    ...
    /*
    * The temp_hit is because we want to store the instance of the HitInfo class,
    * which is closest to the ray we are shooting with.
    * (Additionally I made a did_it_hit boolean, but it probably not necessary).
    */
    ...
    for (unsigned int i = 0; i < primitives.size(); ++i) {
        /*
        * Then we go through every single primitive and check which we hit first.
        * The primitives are instances of the AccObj. This class has a member-variable
        * called geometry which is an instance of the Object3D class.
        * Now: plane, triangle and sphere all inherit from the Object3D class, which means
        * that this geometry variable (an instance of Object3D) has the intersect function
        * ...and that is the complicated way of how we call the intersect function.
        */
        did_it_hit = primitives[i]->geometry->intersect(r, hit, primitives[i]->prim_idx);

        if (did_it_hit) {
            /*
            * We redefine the maximum distance that we wish to check, according
            * to how far away the closest object, so far, has been.
            */
            r.tmax = hit.dist;
